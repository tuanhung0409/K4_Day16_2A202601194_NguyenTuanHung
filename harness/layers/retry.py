...
class Retry(Middleware):
    ...
    def wrap_tool_call(self, ctx, call, name, args):
        ...
        attempts = 1  # đã gọi lần đầu ở trên
        ...
        while attempts < self.max_attempts:
            ...
            # Kiểm tra ngân sách trước khi gọi thêm
            limit = ctx.max_tool_calls
            if limit is not None and ctx.tools.calls >= limit - self.reserve:
                break  # hết ngân sách → dừng, không gọi thêm

            # Gọi lại với đúng name/args cũ
            result = call(name, args)
            attempts += 1

        # Ghi số lần thử vào ctx.state để gỡ lỗi
        ctx.state.setdefault("retry_counts", []).append(attempts)

        # Trả về kết quả cuối cùng dù vẫn hỏng — agent phải thấy sự thật
        return result
